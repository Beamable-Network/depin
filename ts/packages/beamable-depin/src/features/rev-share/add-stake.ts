...
export const AddStakeParamsCodec: Codec<AddStakeParams> = getStructCodec([
    ["amount", getU64Codec()],
]);

export interface CreateAddStakeInput {
    user: Address;
    payer: Address;
    amount: bigint;
...
export class AddStake {
    readonly user: Address;
    readonly payer: Address;
    readonly params: AddStakeParams;
    readonly user_bmb_token_account: Address;
    readonly active_offer_id: number;

    constructor(input: CreateAddStakeInput) {
    ...
    private serialize(): Uint8Array {
        const inner = AddStakeParamsCodec.encode(this.params);
        return Uint8Array.of(RevShareInstruction.AddStake, ...inner);
    }
