...
CHANGELOG_SCRIPT="changelog:sdk"
...
RELATIVE_VERSION_FILE="ts/packages/beamable-depin/$VERSION_FILE"
...
git diff "$RELATIVE_VERSION_FILE" "ts/packages/beamable-depin/CHANGELOG.md"
...
if [[ ! $confirm =~ ^[Yy]$ ]]; then
    ...
    git checkout -- "$RELATIVE_VERSION_FILE" "ts/packages/beamable-depin/CHANGELOG.md"
    exit 1
fi

# Commit
echo -e "\n${BLUE}Creating commit...${NC}"
git add "$RELATIVE_VERSION_FILE" "ts/packages/beamable-depin/CHANGELOG.md"
...
echo -e "\n${BLUE}Next steps:${NC}"
echo "  cd ts/packages/beamable-depin"
echo "  pnpm publish:npm"
