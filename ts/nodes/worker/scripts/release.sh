...
RELATIVE_VERSION_FILE="ts/nodes/worker/$VERSION_FILE"
...
git-cliff --include-path 'ts/nodes/worker/**' \
          --tag-pattern 'worker-v[0-9]*' \
          --tag "${TAG_PREFIX}-v${NEW_VERSION}" \
          --output ts/nodes/worker/CHANGELOG.md
echo -e "${GREEN}✓ Changelog generated${NC}"

# Show changes
echo -e "\n${YELLOW}Changes to be committed:${NC}"
git diff "$RELATIVE_VERSION_FILE" "ts/nodes/worker/CHANGELOG.md"
...
if [[ ! $confirm =~ ^[Yy]$ ]]; then
    ...
    git checkout -- "$RELATIVE_VERSION_FILE" "ts/nodes/worker/CHANGELOG.md"
    exit 1
fi

# Commit
echo -e "\n${BLUE}Creating commit...${NC}"
git add "$RELATIVE_VERSION_FILE" "ts/nodes/worker/CHANGELOG.md"
