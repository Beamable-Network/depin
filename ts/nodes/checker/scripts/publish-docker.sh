...
# Publish script for the Checker Docker image
# Usage: ./scripts/publish-docker.sh [image-name]
# Example: ./scripts/publish-docker.sh dpatekar/checker

set -euo pipefail

IMAGE_NAME="${1:-beamablenetwork/checker}"
...
echo "Publishing DePIN Checker Docker image"
