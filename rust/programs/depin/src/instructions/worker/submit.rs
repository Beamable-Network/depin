...
pub fn process_submit_worker_proof(
    ...
    invoke_signed(
        ...
        &[&[
            crate::shared::constants::seeds::PROOF_SEED,
...
fn update_checker_rewards(
    ...
    // Run BRAND
    let numbers = crate::shared::utils::brand::generate_numbers(
        leaf_asset_id.as_ref(),
        period,
        512,
    ...
    for (array_index, &checker_bits) in checkers.iter().enumerate() {
        ...
        while bits != 0 {
            ...
            GlobalRewards::add_checker_balance(&mut global_rewards_data, checker_index, period_reward_tokens)?;
