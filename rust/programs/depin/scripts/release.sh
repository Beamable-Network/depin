...
CURRENT_VERSION=$(grep '^version = ' "rust/programs/depin/$VERSION_FILE" | head -1 | sed 's/.*"\(.*\)".*/\1/')
...
git diff "$RELATIVE_VERSION_FILE" "rust/programs/depin/CHANGELOG.md"
...
if [[ ! $confirm =~ ^[Yy]$ ]]; then
    ...
    git checkout -- "$RELATIVE_VERSION_FILE" "rust/programs/depin/CHANGELOG.md"
    exit 1
fi

# Commit
echo -e "\n${BLUE}Creating commit...${NC}"
git add "$RELATIVE_VERSION_FILE" "rust/programs/depin/CHANGELOG.md"
