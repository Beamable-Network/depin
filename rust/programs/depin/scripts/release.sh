...
echo -e "${BLUE}=== Beamable ${PACKAGE_NAME} Release ===${NC}\n"
...
RELATIVE_VERSION_FILE="rust/programs/depin/$VERSION_FILE"
sed -i "0,/^version = .*$/s//version = \"$NEW_VERSION\"/" "$RELATIVE_VERSION_FILE"
...
git-cliff --unreleased \
          ...
          --include-path 'rust/depin_core/src/**' \
...
git diff "$RELATIVE_VERSION_FILE" "rust/programs/depin/CHANGELOG.md" "rust/Cargo.lock"
...
if [[ ! $confirm =~ ^[Yy]$ ]]; then
    ...
    git checkout -- "$RELATIVE_VERSION_FILE" "rust/programs/depin/CHANGELOG.md" "rust/Cargo.lock"
    exit 1
fi

# Commit
echo -e "\n${BLUE}Creating commit...${NC}"
git add "$RELATIVE_VERSION_FILE" "rust/programs/depin/CHANGELOG.md" "rust/Cargo.lock"
...
echo -e "\n${BLUE}Next steps:${NC}"
echo "  cd rust/programs/depin"
echo "  cargo build-sbf"
echo "  # Deploy to Solana"
