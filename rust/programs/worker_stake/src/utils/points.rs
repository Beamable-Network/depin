...
mod tests {
    ...
    fn test_calculate_points_balanced() {
        ...
        assert_eq!(calculate_points(3, 7500), 3);
        assert_eq!(calculate_points(1, 2500), 1);
        assert_eq!(calculate_points(10, 25000), 10);
    }

    #[test]
    fn test_calculate_points_capped_by_licenses() {
        // More stake than needed, capped by license count
        assert_eq!(calculate_points(2, 10000), 2);
        assert_eq!(calculate_points(1, 5000), 1);
    }

    #[test]
    fn test_calculate_points_capped_by_stake() {
        // More licenses than stake supports
        assert_eq!(calculate_points(5, 10000), 4);
        assert_eq!(calculate_points(10, 12500), 5);
    }

    #[test]
    fn test_calculate_points_insufficient_stake() {
        // Not enough stake for even one license
        assert_eq!(calculate_points(3, 2000), 0);
        assert_eq!(calculate_points(5, 1000), 0);
    }

    #[test]
    fn test_calculate_points_no_licenses() {
        // No licenses means no points
        assert_eq!(calculate_points(0, 10000), 0);
