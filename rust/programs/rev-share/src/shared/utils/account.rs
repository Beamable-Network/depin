...
use crate::shared::{constants::accounts::DISC_SIZE, types::account::RevShareAccountType};

pub fn write_account_data<T: BorshSerialize>(
    data: &mut [u8],
    discriminator: RevShareAccountType,
    account_data: &T,
) -> Result<(), ProgramError> {
    if data.is_empty() {
...
pub fn read_account_data<'a, T: BorshDeserialize>(
    data: &'a [u8],
    expected_discriminator: RevShareAccountType,
